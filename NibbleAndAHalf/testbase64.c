...
int testBase64(const void* data, int dataLen) {
  ...
  if (dataLen < 1 << 10)
    printf("%d Bytes ", dataLen);
  else if (dataLen < 1 << 20)
    printf("%d KB ", dataLen >> 10);
  else if (dataLen < 1 << 30)
    printf("%d MB ", dataLen >> 20);
  else
    printf("%d GB ", dataLen >> 30);
  puts("of data");
  ...
  if (!base64integrity(
      ...
      ))  // Check the integrity of the base64'd string
  {
  ...
  if (BASE64TESTSHOWDATA) {
    puts("Original data:");
    for (int i = 0; i < dataLen; i++) putc(binaryPtr[i], stdout);
    puts("\n--------------------");
    puts("Base64'd ascii text:");
    puts(base64String);
    puts("--------------------");
    puts("Unbase64'd recovered data:");
    for (int i = 0; i < recoveredLen; i++) putc(recoveredData[i], stdout);
    puts("\n--------------------");
  }
  ...
  // Data is the exact same len. Good.
    ...
    for (int i = 0; i < dataLen; i++)  // good ol' else for
    {
      if (BASE64TESTSHOWDATA) printf("\n%4d  | %3d | %3d |", i, binaryPtr[i], recoveredData[i]);
...
void testUnbase64WithBadAscii() {
  ...
  puts(badAscii);
  for (int i = 0; i < badAsciiLen; i++) printf("%3d, ", badAscii[i]);
  puts("");
  ...
  if (!base64integrity(badAscii, badAsciiLen)) {
    ...
    for (int i = 0; i < baddatLen; i++) printf("%3d, ", baddat[i]);
    puts("");
    free(baddat);
  }
}
...
void testUnbase64WithBadLength() {
  ...
  for (int i = 0; i < binaryLen; i++) printf("%3d, ", binaryData[i]);
  puts("");
}
...
void automatedTests() {
  ...
  for (int testDatLen = 1; testDatLen <= BASE64TESTMAXDATALEN; testDatLen <<= 1) {
    ...
    for (int i = 0; i < testDatLen; i++) dat[i] = rand();  // make new random data
